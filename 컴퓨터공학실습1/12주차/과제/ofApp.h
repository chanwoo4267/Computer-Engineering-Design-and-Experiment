...
class ofApp : public ofBaseApp {
	...
	public:
		...
		bool DFS();
		bool BFS();
		void findbfs(int maze_row, int maze_col, int HEIGHT, int WIDTH);
		void finddfs(int maze_row, int maze_col, int HEIGHT, int WIDTH);
		void dfsdraw();
		void bfsdraw();
		int HEIGHT;//미로의 높이
		int WIDTH;//미로의 너비
		char* input[10000];//텍스트 파일의 모든 정보를 담는 이차원 배열이다.
		int load_flag = 0;
